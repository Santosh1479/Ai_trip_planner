...
export default function TripDetails() {
  ...
  const days =
    tripData.days ||
    (tripData.itinerary && tripData.itinerary[0]?.days) ||
    [];

  const getDayPath = (day) =>
    day.path || day.activities?.map((a) => ({ ...a, type: "activity" })) || [];

  const sanitizeCity = (location) => {
    if (!location) return "";
    // Remove 'Hotel in' if present
    return location.replace(/Hotel in\s*/i, "").split(",")[0].trim();
  };

  const getFlightUrl = (day, nextDay, travelers) => {
    const origin = sanitizeCity(day.path[0]?.location);
    const destination = sanitizeCity(nextDay.path[0]?.location);
    const departure_date = day.date; // YYYY-MM-DD
    const adults = travelers || 1;

    return `https://www.booking.com/flights/index.en-gb.html?origin=${encodeURIComponent(
      origin
  ...
  const getHotelUrl = (day) => {
    const city = sanitizeCity(day.path[0]?.location);
    if (!city) return "#";
      ...
      {days.length > 0 ? (
        days.map((day, idx) => (
          <div key={idx} className="mb-12">
            ...
            {/* Hotel Booking Button for this day */}
            <div className="mt-4">
              <a
                href={getHotelUrl(day)}
                target="_blank"
                rel="noopener noreferrer"
                className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 mt-3 inline-block"
              >
                🏨 Book Hotel in {sanitizeCity(day.path[0]?.location)}
              </a>
            </div>

            {/* Flight Booking Button if next day's location differs */}
            {days[idx + 1] &&
              sanitizeCity(day.path[0]?.location) !==
                sanitizeCity(days[idx + 1].path[0]?.location) && (
                <div className="mt-2">
                  <a
                    href={getFlightUrl(day, days[idx + 1], tripData.tripSummary?.travelers)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 mt-3 inline-block"
                  >
                    ✈️ Book Flight: {sanitizeCity(day.path[0]?.location)} →{" "}
                    {sanitizeCity(days[idx + 1].path[0]?.location)}
                  </a>
                </div>
              )}
